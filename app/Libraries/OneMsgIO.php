...
class OneMsgIO
{
    ...
    private function execOneMsgAPI($jsonDataParam, $endPoint = 'sendMessage')
    {
        ...
        try {
            ...
            if(!$isSent) {
                ...
                log_message('info', 'OneMsgIO Send Message Error: '.$errorMsg);
    ...
    public function getListOfTemplates()
    {
        ...
        try {
            $response = curl_exec($ch);
            log_message('info', 'OneMsgIO Template List Response: '.$response);
            if (curl_errno($ch)) {
                return [];
            } else {
    ...
    {
        if(isset($templateList) && isset($templateList['templates']) && is_array($templateList) && is_array($templateList['templates']) && count($templateList['templates']) > 0){
