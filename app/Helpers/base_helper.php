...
if (!function_exists('issetNotNullAndNotEmptyString')) {
    /**
     ...
     * @return boolean
     */
    function issetNotNullAndNotEmptyString($variable, $defautlValue = null){
        return isset($variable) && !is_null($variable) && $variable !== '' ? $variable : $defautlValue;
    }
}
...
if (!function_exists('validateHeadersRequiredParams')) {
    /**
     * Validate HardwareID and TimeZoneOffset headers from request
     * Only allows GMT+7 (420), GMT+8 (480), GMT+9 (540)
     * Defaults to GMT+7 (420) if not in allowed list
     *
     * @return array Returns ['valid' => true, 'data' => [...]] on success or ['valid' => false, 'errors' => [...]] on failure
     */
    function validateHeadersRequiredParams() {
        $request        =   \Config\Services::request();
        $validation     =   \Config\Services::validation();
        $hardwareID     =   $request->getHeaderLine('HardwareID');
        $timezoneOffset =   $request->getHeaderLine('TimeZoneOffset');
        $platform       =   $request->getHeaderLine('Platform');

        // Allowed timezone offsets in minutes: GMT+7, GMT+8, GMT+9
        $allowedTimezones   =   [420, 480, 540];
        $defaultTimezone    =   420; // GMT+7
        ...
        $rules  =   [
            'HardwareID'    => 'required|alpha_numeric|min_length[10]',
            'TimeZoneOffset'=> 'required|integer',
            'Platform'      => 'required|in_list[M,W]'
        ];
        
        $messages = [
            'HardwareID'    =>  [
                'required'      =>  '[E-AUTH-000.1.1] Gagal memproses data, harap buka ulang aplikasi atau hubungi tim support kami',
                'alpha_numeric' =>  '[E-AUTH-000.1.2] Gagal memproses data, harap buka ulang aplikasi atau hubungi tim support kami',
                'min_length'    =>  '[E-AUTH-000.1.3] Gagal memproses data, harap buka ulang aplikasi atau hubungi tim support kami'
            ],
            'TimeZoneOffset'=>  [
                'required'  =>  '[E-AUTH-000.2.1] Gagal memproses data, harap buka ulang aplikasi atau hubungi tim support kami',
                'integer'   =>  '[E-AUTH-000.2.2] Gagal memproses data, harap buka ulang aplikasi atau hubungi tim support kami'
            ],
            'Platform' =>  [
                'required'  =>  '[E-AUTH-000.3.1] Gagal memproses data, harap buka ulang aplikasi atau hubungi tim support kami',
                'in_list'   =>  '[E-AUTH-000.3.2] Gagal memproses data, harap buka ulang aplikasi atau hubungi tim support kami'
            ]
        ];
        ...
        // Validate timezone is in allowed list, if not set to default GMT+7
        $timezoneOffsetInt = (int)$timezoneOffset;
        if (!in_array($timezoneOffsetInt, $allowedTimezones)) {
            $timezoneOffsetInt = $defaultTimezone;
        }
