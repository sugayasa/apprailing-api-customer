...
class Auth implements FilterInterface
{
    ...
    public function before(RequestInterface $request, $arguments = null)
    {
        ...
        if(!$header) return throwResponseUnauthorized('[E-AUTH-000] Token wajib disertakan');
        $arrHeader          =   $header != "" ? explode(' ', $header) : [];
        $token              =   count($arrHeader) > 0 && isset($arrHeader[1]) ? $arrHeader[1] : "";
        $hardwareIDHeader   =   $request->getHeaderLine('HardwareID');
        ...
        try {
            ...
            $request->token             =   $token;
            $request->hardwareIDHeader  =   $hardwareIDHeader;
            ...
            $idCustomer                 =   $dataDecode->idCustomer;
            ...
            if(isset($arguments) && $arguments[0] == 'mustNotBeLoggedIn'){
                if(isset($idSession) && $idSession != "" && intval($idSession) != 0){
                    return throwResponseUnauthorized('Anda tidak diizinkan melakukan aksi ini karena sesi login Anda sudah aktif');
                }
            } else if(isset($arguments) && $arguments[0] == 'mustBeLoggedIn'){
                if(!isset($idSession) || $idSession == "" || intval($idSession) == 0){
                    return throwResponseUnauthorized('Silakan masuk untuk melakukan aksi ini');
                }
